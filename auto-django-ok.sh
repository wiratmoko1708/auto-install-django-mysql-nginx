...
# Updated with Fixes: Socket Path, PyMySQL, CSRF, Static Files
# ==========================================
...
step_install_db() {
    ...
    # Coba ubah password root (support various MariaDB versions)
...
step_setup_django() {
    ...
    # [FIX] Pastikan www-data memiliki akses ke parent directory jika diperlukan, 
    # tapi biasanya /var/www/ milik root, kita akan chown $APP_DIR nanti.
    ...
    # [FIX] Tambahkan pymysql sebagai antisipasi masalah mysqlclient
    log_info "Menginstall Django dan dependensi..."
    pip install --upgrade pip
    pip install django gunicorn mysqlclient pymysql
    ...
    # [FIX] Konfigurasi PyMySQL sebagai MySQLdb di __init__.py
    INIT_FILE="$APP_DIR/$PROJECT_NAME/$PROJECT_NAME/__init__.py"
    if [ -f "$INIT_FILE" ]; then
        log_info "[SKIP] Menambahkan PyMySQL ke $INIT_FILE..."
        # Cek apakah sudah ada
        if ! grep -q "pymysql.install_as_MySQLdb()" "$INIT_FILE"; then
            echo "" >> "$INIT_FILE"
            echo "import pymysql" >> "$INIT_FILE"
            echo "pymysql.install_as_MySQLdb()" >> "$INIT_FILE"
        fi
    fi
...
EOF
    ...
    if [ -f "$SETTINGS_FILE" ]; then
        log_info "Mengkonfigurasi settings.py untuk MySQL dan Security..."
...
# Tambah STATIC_ROOT (Pastikan menggunakan 'staticfiles' agar konsisten dengan Nginx)
if "STATIC_ROOT" not in content:
    content += "\nSTATIC_ROOT = BASE_DIR / 'staticfiles'\n"
else:
    # Jika sudah ada, pastikan mengarah ke staticfiles jika perlu, 
    # atau biarkan user mengatur sendiri. Di sini kita append saja jika belum ada.
    pass
...
PYEOF
        ...
        # [FIX] Tambahkan Security & CSRF Settings manual (Python script di atas regex-based, ini append simple)
        cat >> "$SETTINGS_FILE" <<EOF

# ==============================================
# Security & CSRF Settings (Auto-Added)
# ==============================================
...
EOF
    ...
    log_info "Mengumpulkan static files..."
    "$APP_DIR/venv/bin/python" manage.py collectstatic --noinput 2>/dev/null || true

    # Set permissions
    # [FIX] Pastikan www-data memiliki akses penuh ke direktori proyek
...
step_configure_gunicorn() {
    ...
    # [FIX] Gunakan socket di /var/www/... bukan di /run/ untuk menghindari permission issue
    SOCKET_FILE="$APP_DIR/gunicorn.sock"

    # Buat socket file config
...
step_configure_nginx() {
    ...
    # [FIX] Pastikan proxy_pass mengarah ke socket file yang baru ($APP_DIR/gunicorn.sock)
    SOCKET_FILE="$APP_DIR/gunicorn.sock"
...
step_show_status() {
    ...
    # echo "Django:     $($APP_DIR/venv/bin/python -m django --version 2>/dev/null)" # May fail if deactivated
    ...
    echo "Gunicorn Socket:   $APP_DIR/gunicorn.sock"
